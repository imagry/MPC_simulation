...
scenario = 'sin' # 'sin', 'straight_line', 'square', shiba
...
while not stop_condition:
    ...
    vehicle_obj.update(a=0, delta=SC.delta)
    # store values
    x.append(vehicle_obj.x)
    y.append(vehicle_obj.y)
    dx = x[-1] - x[-2]
    ...
    if np.mod(i, ndt) == 0 and simulation_params['animate']:
        ...
        vehicle_line = Functions.draw_car(vehicle_obj.x, vehicle_obj.y, vehicle_obj.psi, steer=SC.delta, car_params=vehicle_params, ax=vehicle_animation_axis)
        vehicle_animation_axis.set_xlim(vehicle_obj.x - 20, vehicle_obj.x + 20)
        vehicle_animation_axis.set_ylim(vehicle_obj.y - 20, vehicle_obj.y + 20)
        vehicle_animation_axis.grid(True)
        lateral_error_axis.clear()
        lateral_error_axis.plot(ef)
        lateral_error_axis.grid(True)
